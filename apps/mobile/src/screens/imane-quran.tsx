...
function getFallbackSurahs(language: "fr" | "en"): QuranSurahSummary[] {
  return language === "en" ? FALLBACK_SURAHS_EN : FALLBACK_SURAHS_FR;
}

function useQuranSurahs(language: "fr" | "en") {
  const fallback = useMemo(() => getFallbackSurahs(language), [language]);
  const [surahs, setSurahs] = useState<QuranSurahSummary[]>(fallback);
  ...
  const loadSurahs = useCallback(async () => {
    ...
    try {
      ...
      const list = response?.surahs?.length
        ? response.surahs
        : getFallbackSurahs(language);
      const didFallback = !response?.surahs?.length;
      setSurahs(list);
      setUsingFallback(didFallback);
      setSelectedSurahId((prev) => prev ?? list[0]?.id ?? null);
    } catch (err) {
      const message =
      ...
      setError(message);
      const fb = getFallbackSurahs(language);
      setSurahs(fb);
      setUsingFallback(true);
      setSelectedSurahId((prev) => prev ?? fb[0]?.id ?? null);
  ...
  // Update fallback surahs when language changes
  useEffect(() => {
    setSurahs((prev) =>
      prev.length === 0 ? getFallbackSurahs(language) : prev,
    );
  }, [language]);
...
const JUZ_TO_SURAH = [
  ...
  { juz: 2, startSurah: 2, startAyah: 142, label: "Al-Baqara 142—252" },
  {
    ...
    startSurah: 2,
    startAyah: 253,
    label: "Al-Baqara 253 — Al-Imran 92",
  },
  ...
  { juz: 18, startSurah: 23, startAyah: 1, label: "Al-Muminun — Al-Furqan 20" },
  {
    ...
    startSurah: 25,
    startAyah: 21,
    label: "Al-Furqan 21 — An-Naml 55",
  },
  {
    juz: 20,
    startSurah: 27,
    startAyah: 56,
    label: "An-Naml 56 — Al-Ankabut 45",
  },
  {
    juz: 21,
    startSurah: 29,
    startAyah: 46,
    label: "Al-Ankabut 46 — Al-Ahzab 30",
  },
  ...
  { juz: 23, startSurah: 36, startAyah: 28, label: "Ya-Sin 28 — Az-Zumar 31" },
  {
    ...
    startSurah: 39,
    startAyah: 32,
    label: "Az-Zumar 32 — Fussilat 46",
  },
  {
    juz: 25,
    startSurah: 41,
    startAyah: 47,
    label: "Fussilat 47 — Al-Jathiya 37",
  },
  {
    juz: 26,
    startSurah: 46,
    startAyah: 1,
    label: "Al-Ahqaf — Adh-Dhariyat 30",
  },
  {
    juz: 27,
    startSurah: 51,
    startAyah: 31,
    label: "Adh-Dhariyat 31 — Al-Hadid",
  },
...
export function ImaneQuranScreen({
    ...
    SecureStore.getItemAsync(fontSizeKey)
      .then((v) => {
        if (v !== null) {
          ...
          if (Number.isFinite(n) && n >= 0 && n < FONT_SIZES.length)
            setFontSizeIndex(n);
        }
      })
      .catch(() => {});
  ...
  const setFontSizeIndexPersisted = useCallback(
    (idx: number) => {
  ...
  const {
    ...
    loadingVerses,
    error: versesError,
  } = useQuranVerses({ selectedSurahId, language });
  ...
  const [tafsirTextByKey, setTafsirTextByKey] = useState<
  ...
  const loadAndPlayTrack = useCallback(
    async (autoPlay = false) => {
      ...
      setAudioError(null);
      try {
        const response = await quranApi.getSurahAudio(
          selectedSurahId,
          selectedReciter,
        );
        ...
        const message =
          err instanceof Error ? err.message : "Impossible de charger l'audio.";
  ...
  const seekAudio = useCallback(
    async (offsetMs: number) => {
      const nextSec = Math.max(
        0,
        Math.min(progress.duration, progress.position + offsetMs / 1000),
      );
  ...
  useEffect(() => {
    selectedSurahIdRef.current = selectedSurahId;
  }, [selectedSurahId]);

  useTrackPlayerEvents([Event.PlaybackQueueEnded], () => {
    if (
      continuousPlaybackRef.current &&
      selectedSurahIdRef.current !== null &&
      selectedSurahIdRef.current < 114
    ) {
        ...
        const cleaned: QuranBookmark[] = parsed
          .map((item) => {
            ...
            if (
              ...
              !Number.isFinite(candidate.ayah)
            )
              return null;
  ...
  useEffect(() => {
    ...
    const targetAyah =
      juzTargetAyah ??
      (hydrated && lastRead && selectedSurahId === lastRead.surahId
        ? lastRead.ayah
        : null);
  ...
  const bookmarkKeyFor = useCallback(
    (surahId: number, ayah: number) => `${surahId}:${ayah}`,
    [],
  );
  ...
  useEffect(() => {
    SecureStore.getItemAsync(readSurahsKey)
      .then((raw) => {
        if (!raw) return;
        try {
  ...
  const sortedBookmarks = useMemo(() => {
    return [...bookmarks].sort(
      (a, b) => a.surahId - b.surahId || a.ayah - b.ayah,
    );
  }, [bookmarks]);
    ...
    (surahId: number, ayah: number) => {
      setBookmarks((prev) => {
        ...
        const exists = prev.some(
          (b) => bookmarkKeyFor(b.surahId, b.ayah) === key,
        );
        const next = exists
          ? prev.filter((b) => bookmarkKeyFor(b.surahId, b.ayah) !== key)
          : [...prev, { surahId, ayah }];
  ...
  const handleShowTafsir = useCallback(
    async (surahId: number, ayahNumber: number) => {
      ...
      try {
        ...
        setTafsirTextByKey((prev) =>
          prev[key] === response.text
            ? prev
            : { ...prev, [key]: response.text },
        );
      } catch (err) {
      ...
      <View style={[q.screen, { paddingTop: insets.top }]}>
        ...
        <View style={q.topBar}>
          ...
          <View style={{ flexDirection: "row", alignItems: "center", gap: 6 }}>
            <HelpTip
              screenName="Lire le Coran"
              tips={[
                {
                  icon: "book",
                  title: "Parcourir les sourates",
                  description:
                    "Sélectionne une sourate dans la liste pour lire ses versets avec traduction.",
                },
                {
                  icon: "language",
                  title: "Langue",
                  description:
                    "Bascule entre français et anglais avec le bouton FR/EN.",
                },
                {
                  icon: "text",
                  title: "Taille de police",
                  description:
                    "Ajuste la taille du texte arabe et de la traduction avec le curseur.",
                },
                {
                  icon: "bookmark",
                  title: "Signets",
                  description:
                    "Appuie sur l'icône signet pour marquer un verset. Retrouve tes signets dans le panneau dédié.",
                },
                {
                  icon: "play",
                  title: "Audio",
                  description:
                    "Appuie sur l'icône lecture pour écouter la récitation du verset.",
                },
                {
                  icon: "time",
                  title: "Dernière lecture",
                  description:
                    "Ta position de lecture est sauvegardée automatiquement.",
                },
              ]}
            />
        ...
        <View style={q.readingIntroBanner}>
          ...
          <View style={{ flex: 1 }}>
            ...
            <Text style={q.readingIntroSub}>
              Appuie sur une sourate pour lire avec traduction
            </Text>
          </View>
          {readSurahs.size > 0 && (
        ...
        <View style={{ flexDirection: "row", gap: 8, paddingHorizontal: 16 }}>
          <View
            style={[
              q.searchContainer,
              { flex: 1, marginHorizontal: 0, marginVertical: 10 },
            ]}
          >
            <Ionicons
              name="search"
        ...
        {lastRead && hydrated && (
          <TouchableOpacity
            ...
            onPress={() => {
              handleSelectSurah(lastRead.surahId);
            }}
          >
            <Ionicons name="bookmark" size={16} color="#fff" />
            <Text style={q.lastReadText}>
              Continuer :{" "}
              {surahNameById.get(lastRead.surahId) ??
                `Sourate ${lastRead.surahId}`}{" "}
              · Verset {lastRead.ayah}
            </Text>
            <Ionicons
        ...
        {!loadingSurahs && (surahError || usingFallback) && (
          <View
            style={{
              flexDirection: "row",
              alignItems: "center",
          ...
          >
            ...
            <Text style={{ flex: 1, fontSize: 12, color: "#856404" }}>
              {surahError
                ? "Mode hors-ligne — données locales"
                : "Données locales utilisées"}
            </Text>
            <TouchableOpacity
              onPress={reloadSurahs}
              ...
              return (
                ...
                >
                  ...
                  <View style={{ flex: 1, gap: 2 }}>
                    ...
                    <Text style={q.surahNameTranslated}>
    ...
    <View style={[q.screen, { paddingTop: insets.top }]}>
      ...
      <View style={q.readingBar}>
        ...
        <View style={{ flex: 1, alignItems: "center" }}>
          <Text style={q.readingTitle} numberOfLines={1}>
            {selectedSurah?.nameSimple ?? ""}
          </Text>
          <Text style={q.readingSubtitle}>
        ...
        <View style={[ss.row, ss.gap6]}>
          <TouchableOpacity
            style={q.iconBtn}
            onPress={() => setMemorizationMode((v) => !v)}
          >
            <Ionicons
              name={memorizationMode ? "eye-off" : "eye"}
              size={18}
              color={memorizationMode ? q_c.accent : q_c.muted}
            />
          </TouchableOpacity>
          <TouchableOpacity
            style={q.iconBtn}
        ...
        <View style={q.surahNavBar}>
          ...
          <TouchableOpacity
            style={[
              q.surahNavBtn,
              q.surahNavBtnRight,
              selectedSurahId >= 114 && { opacity: 0.3 },
            ]}
            onPress={() => {
      ...
      {audioDuration > 0 && (
        <View style={{ paddingHorizontal: 20, marginBottom: 4 }}>
          <View style={q.audioProgressTrack}>
            <View
              style={[
                q.audioProgressFill,
                {
                  width: `${Math.min(100, (audioPosition / audioDuration) * 100)}%`,
                },
              ]}
            />
          </View>
          ...
          >
            <Text style={{ fontSize: 11, color: q_c.muted }}>
      ...
      <View style={q.fontBar}>
        <TouchableOpacity
          ...
          onPress={() =>
            setFontSizeIndexPersisted(Math.max(0, fontSizeIndex - 1))
          }
        ...
        <TouchableOpacity
          style={[
            q.fontBtn,
            fontSizeIndex === FONT_SIZES.length - 1 && { opacity: 0.3 },
          ]}
          onPress={() =>
            setFontSizeIndexPersisted(
              Math.min(FONT_SIZES.length - 1, fontSizeIndex + 1),
            )
          }
      ...
      {audioError && (
        <Text
          style={[ss.errorText, { paddingHorizontal: 20, marginBottom: 4 }]}
        >
          {audioError}
        </Text>
      )}
        ...
        >
          ...
          {filteredVerses.map((verse) => {
            const isLastRead =
              ...
              lastRead?.ayah === verse.verseNumber;
            const isBookmarked =
              selectedSurahId !== null &&
              bookmarkSet.has(
                bookmarkKeyFor(selectedSurahId, verse.verseNumber),
              );
            return (
              <View
                ...
                onLayout={(e) => {
                  ...
                  setVerseOffsets((prev) =>
                    prev[verse.verseNumber] === y
                      ? prev
                      : { ...prev, [verse.verseNumber]: y },
                  );
                }}
              >
                  ...
                  <View style={[ss.row, ss.gap8]}>
                    ...
                    >
                      <Ionicons
                        name="eye-outline"
                        size={18}
                        color={isLastRead ? q_c.accent : q_c.muted}
                      />
                    </TouchableOpacity>
                    {selectedSurahId !== null && (
                      <>
                        <TouchableOpacity
                          ...
                          disabled={
                            tafsirLoadingKey ===
                            `${selectedSurahId}:${verse.verseNumber}:${language}`
                          }
                        >
                          <Ionicons
                            name="book-outline"
                        ...
                        >
                          <Ionicons
                            ...
                            size={18}
                            color={isBookmarked ? q_c.accent : q_c.muted}
                          />
                        </TouchableOpacity>
                      </>
                    )}
                ...
                {!memorizationMode && verse.textTransliteration && (
                  <Text
                    style={[
                      q.transliterationText,
                      {
                        fontSize: currentFont.trans - 1,
                        lineHeight: (currentFont.trans - 1) * 1.5,
                      },
                    ]}
                  >
                ...
                {!memorizationMode && verse.textTranslated && (
                  <Text
                    style={[
                      q.translationText,
                      {
                        fontSize: currentFont.trans,
                        lineHeight: currentFont.trans * 1.6,
                      },
                    ]}
                  >
                    {verse.textTranslated}
      ...
      {sortedBookmarks.length > 0 && (
        <View style={q.bookmarkFab}>
          <TouchableOpacity
            style={q.fabBtn}
            onPress={() => setShowBookmarksPanel(true)}
          >
            ...
            >
              ...
              >
                ...
                <View
                  style={[
                    q.continuousToggle,
                    continuousPlayback && q.continuousToggleOn,
                  ]}
                >
                  <Ionicons
                    name={continuousPlayback ? "repeat" : "repeat-outline"}
                    size={16}
                    color={continuousPlayback ? "#fff" : q_c.muted}
                  />
                </View>
              </TouchableOpacity>
              ...
              {RECITERS.map((reciter) => (
                <TouchableOpacity
                  ...
                  style={[
                    q.reciterItem,
                    selectedReciter === reciter.id && q.reciterItemActive,
                  ]}
                  onPress={() => {
      ...
      {showBookmarksPanel && (
        <View style={q.bookmarksPanel}>
          <View style={q.bookmarksPanelHeader}>
            <Text style={q.bookmarksPanelTitle}>
              Mes signets ({sortedBookmarks.length})
            </Text>
