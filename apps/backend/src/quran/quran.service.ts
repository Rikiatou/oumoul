...
export class QuranService {
  ...
  async listSurahs(language: string = 'fr'): Promise<QuranSurah[]> {
    ...
    if (!json.chapters || !Array.isArray(json.chapters)) {
      throw new InternalServerErrorException('Réponse inattendue de Quran.com pour la liste des sourates');
    }
  ...
  async getSurahVerses(chapterId: number, language: string = 'fr'): Promise<QuranVerse[]> {
    const normalizedLang = language === 'en' ? 'en' : 'fr';
    const translationEdition = normalizedLang === 'en' ? 'en.sahih' : 'fr.hamidullah';
    ...
    if (!arabicResponse.ok) {
      throw new InternalServerErrorException('Erreur lors de la récupération des versets arabes depuis AlQuran Cloud');
    }
    ...
    if (!Array.isArray(arabicAyahs) || !Array.isArray(translationAyahs)) {
      throw new InternalServerErrorException('Réponse inattendue de AlQuran Cloud pour les versets');
    }
  ...
  async getSurahAudioUrl(chapterId: number, reciterId: string = 'mishary'): Promise<string> {
    ...
    const baseByReciter: Record<string, string> = {
      ...
      sudais: 'https://server10.mp3quran.net/sds',
      minshawi: 'https://server7.mp3quran.net/minsh',
    };
  ...
  async getAyahTafsir(surah: number, ayah: number, tafsirId: number = 169): Promise<{ text: string }> {
    ...
    if (!json.tafsir || !json.tafsir.text) {
      throw new InternalServerErrorException('Réponse inattendue de Quran.com pour le tafsir');
    }
